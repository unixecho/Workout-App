    ...
    <main style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", paddingBottom: "calc(var(--safe-bottom) + 16px)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
          {e.formCues && (
            <div style={block("blue")}>
              <b style={{ color: "var(--blue)" }}>{t.library.form}</b>
              {e.formCues}
            </div>
          )}
          {e.mistakes.length > 0 && (
            <div style={block("blue")}>
              <b style={{ color: "var(--blue)" }}>{t.library.commonMistakes}</b>
          ...
          {e.adaptation && (
            <div style={block("amber")}>
              <b style={{ color: "var(--amber)" }}>{t.library.forYou}</b>
              {e.adaptation}
            </div>
          )}
        </div>
      ...
      {pickDayId && (
        <div style={{ padding: "16px 20px 0" }}>
          ...
          >
            {pending ? t.library.adding : t.library.addToSession}
          </button>
        </div>
      )}
    </main>
